-- Migration 027: Security Hardening Sprint (RPC Restriction, In-place Patching & Recruiter RLS Allowlist)
-- Run this in InsForge SQL Editor

-- 1. Redefine function to patch internal actor validations in-place
...
-- 2. Restrict execute permissions on RPC
REVOKE EXECUTE ON FUNCTION public.update_application_status(uuid,text,uuid,text,jsonb) FROM public;
REVOKE EXECUTE ON FUNCTION public.update_application_status(uuid,text,uuid,text,jsonb) FROM anon;
REVOKE EXECUTE ON FUNCTION public.update_application_status(uuid,text,uuid,text,jsonb) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.update_application_status(uuid,text,uuid,text,jsonb) TO project_admin;

-- 3. Recruiter access policy for candidate_profiles (status allowlist)
...
-- 4. Recruiter access policy for profiles (status allowlist)
...
-- 5. Recruiter access policy for candidate_resumes (status allowlist)
