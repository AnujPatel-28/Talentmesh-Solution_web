...
async function main() {
  console.log('--- Applying Database Migration: Notification Queue (ESM) ---');
  
  const sqlPath = path.resolve(__dirname, '../insforge/migrations/014_notification_queue.sql');
  ...
  try {
    ...
    console.log('✅ Success: Notification queue tables, triggers, and preferences initialized successfully.');
  } catch (err) {
    console.error('Unexpected error during RPC execution:', err.message || err);
    process.exit(1);
  }
}
