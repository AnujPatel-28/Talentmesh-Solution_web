...
async function main() {
  console.log('--- Applying Database Migration: Cleanup Idempotency Keys Scheduled Job (ESM) ---');
  
  const sqlPath = path.resolve(__dirname, '../insforge/migrations/012_cleanup_idempotency_keys.sql');
  ...
  try {
    ...
    console.log('✅ Success: Daily idempotency keys cleanup cron migration applied successfully.');
  } catch (err) {
